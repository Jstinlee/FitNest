...
<head>
  ...
  <title>FitNest - Review Trainer</title>
...
<nav class="navbar navbar-toggleable-md navbar-inverse nav">
  ...
  <div class="jumbotron">
      <div class="container">
	 <h22>Review Trainer</h22><br/>
	 <p>Please rate and leave a comment on your trainer</p>
</div>
</div>
</header>
...
<h3><?php
  ...
  echo $row['title'];
  ?></h3><br/><br/>
  <form action="reviewSite.php" method="POST">
<div class="row">
    <div class="col-md-6">
		  <div class="trainerdesc">
        <?php
        $query = "SELECT username FROM trainingSession WHERE sessionID = '$id'";
        $result = mysqli_query($mysqli,$query);
        $row = mysqli_fetch_assoc($result);
        $check = $row['username'];
      ...
      $query2 = "SELECT name FROM users WHERE username = '$check'";
      $result2 = mysqli_query($mysqli,$query2);
      $row2 = mysqli_fetch_assoc($result2);
      $fullname = $row2['name'];
      echo "Trainer`s name: $fullname";
      ?></h5>
			<h5>Specialty: <?php
            $query = "SELECT specialty FROM users WHERE username = '$check'";
            $result = mysqli_query($mysqli,$query);
            $row = mysqli_fetch_assoc($result);
      ...
      <input class="star star-5" id="star-5" type="radio" name="star" value="5"/>
		...
		<div class="group">
          <textarea rows="3" name="comment" required></textarea><span class="highlight"></span><span class="bar"></span>
		  <label class="field">Comment</label>
        </div>
		<br>
		<div class="group">
              <button type="submit" name="reviewed" class="btn btn-orange col-md-12">Submit Review</button>
        </div>
    </form>
    </div>
</div>
