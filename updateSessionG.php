...
<head>
  ...
  <title>FitNest - Update Group Training Session</title>
</head>
...
<nav class="navbar navbar-toggleable-md navbar-inverse nav">
 ...
 <div class="jumbotron">
  ...
  <h22>Update Group Training Session</h22>
...
<div class="container">
  <div class="row">
	...
	<form method="POST" action="updateSessionG.php">
		<div class="group">
		...
		<div class="group">
          ...
          $result = mysqli_query($mysqli,$query);
          $row = mysqli_fetch_assoc($result);
          echo $row['fee'];?>" /><span class="highlight"></span><span class="bar"></span>
		...
		<div class="group">
		 ...
		 <?php $query = "SELECT classType FROM trainingSession WHERE sessionID = '$id'";
		 $result = mysqli_query($mysqli, $query);
		 $row = mysqli_fetch_assoc($result);?>
		  <select name="classtype" class="col-md-12">
		  <option value="Dance" <?php if ($row['classType'] == 'Dance') echo "selected='selected'"?>>Dance</option>
		  <option value="MMA" <?php if ($row['classType'] == 'MMA') echo "selected='selected'"?>>MMA</option>
		  <option value="Sport" <?php if ($row['classType'] == 'Sport') echo "selected='selected'"?>>Sport</option>
		  </select>
        </div>
		<div class="group">
              <input type="text" name="max" disabled value="<?php
              $query = "SELECT max FROM trainingSession WHERE sessionID = '$id'";
              $result = mysqli_query($mysqli,$query);
              $row = mysqli_fetch_assoc($result);
              echo $row['max'];?>" /><span class="highlight"></span><span class="bar"></span>
    		  <label class="field">Max Participant</label>
        </div>
		<div class="group">
              <input type="text" name="max" disabled value="<?php
              $query = "SELECT max FROM trainingSession WHERE sessionID = '$id'";
              ...
              $row = mysqli_fetch_assoc($result);
              echo $row['max'];?>" /><span class="highlight"></span><span class="bar"></span>
    		  <label class="field">Number of Participant Enrolled</label>
        </div>
		<div class="group">
        <button type="submit" name="updateG" class="btn btn-orange col-md-12">Update Changes</button>
        </div>
	</form>
	</div>
  </div>
