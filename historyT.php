...
<nav class="navbar navbar-toggleable-md navbar-inverse nav">
    ...
    <div class="collapse navbar-collapse" id="navbarsExampleDefault">
      <ul class="navbar-nav">
        ...
        <li class="nav-item dropdown">
          ...
          <div class="dropdown-menu">
            <a class="dropdown-item" href="newTraining.php">Record New Training Session</a>
			<div class="dropdown-divider"></div>
  ...
  <div class="jumbotron">
      <div class="container">
	 <h22>Training History</h22><br/>
	<p>You may update your training sessions or read reviews of your completed sessions from trainees.</p>
</div>
</div>
</header>
...
<div class="row">
  ...
  $user = $_SESSION['username'];
    ...
    <form method='POST' action='historyT.php'>
    <input type='text' class='hide' name='sessionID' value='$row[0]'>
    <button type='submit' name='updateSession' class='btn btn-orange col-md-12'>Update Session</button></div>
    </form>
		</div>
    ";
  }
	...
	<div class="papercard">
		<?php
  $mysqli = new mysqli("localhost","root","","a2");
	$array = mysqli_query($mysqli, "SELECT * FROM trainingSession WHERE userName = '$user' AND (status = 'Completed' OR status = 'Cancelled')");
	if (mysqli_num_rows($array)==0){
		echo "		<div class='paper'>
    ...
    <form method='POST' action='historyT.php'>
    <input type='text' class='hide' name='sessionID' value='$row[0]'>
    <button type='submit' name='readReview' class='btn btn-orange col-md-12'>Read Review</button></div>
    </form>
		</div>
    ";
  }
